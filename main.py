...
if clientID != -1:
    ...
    [erro, ventoinha] = sim.simxGetObjectHandle(
        clientID, VENTOINHA, sim.simx_opmode_blocking
    )
    if erro != 0:
        ...
        sim.simxFinish(-1)
        exit()
    [erro, corpo_ventoinha] = sim.simxGetObjectHandle(
        clientID, CORPO_VENTOINHA, sim.simx_opmode_blocking
    )
    if erro != 0:
        print("Erro ao solicitar controle de objeto do Coppelia!")
        sim.simxStopSimulation(clientID, sim.simx_opmode_oneshot_wait)
        sim.simxFinish(-1)
        exit()
    [erro, pa_ventoinha] = sim.simxGetObjectHandle(
        clientID, PA_VENTOINHA, sim.simx_opmode_blocking
    )
    if erro != 0:
        print("Erro ao solicitar controle de objeto do Coppelia!")
        sim.simxStopSimulation(clientID, sim.simx_opmode_oneshot_wait)
    ...
    for i in range(0, 6):
        [erro, aux] = sim.simxGetObjectHandle(
            clientID, MOSQUITO + "[" + str(i) + "]", sim.simx_opmode_blocking
        )
        if erro != 0:
            print("Erro ao solicitar controle de objeto do Coppelia!")
            sim.simxStopSimulation(clientID, sim.simx_opmode_oneshot_wait)
    ...
    for i in range(0, 6):
        [erro, aux] = sim.simxGetObjectHandle(
            clientID, CORPO_MOSQUITO + str(i), sim.simx_opmode_blocking
        )
        if erro != 0:
            print("Erro ao solicitar controle de objeto do Coppelia!")
            sim.simxStopSimulation(clientID, sim.simx_opmode_oneshot_wait)
    ...
    def mosquitos(self):
        ...
        while True:
            ...
            [erro, posicao] = sim.simxGetObjectPosition(
                clientID, pa_ventoinha, -1, sim.simx_opmode_blocking
            )
            # print("posicao ventoinha: " + str(posicao))
            sim.simxSetObjectPosition(
                clientID, mosquito[0], -1, posicao, sim.simx_opmode_oneshot_wait
            )
            ...
            posicao = [7.0, 2.0, 1.0]
            sim.simxSetObjectPosition(
                clientID, mosquito[1], -1, posicao, sim.simx_opmode_oneshot_wait
            )
            ...
            sim.simxSetObjectPosition(
                clientID, mosquito[2], -1, posicao, sim.simx_opmode_oneshot_wait
            )
            sim.simxSetObjectPosition(
                clientID, mosquito[3], -1, posicao, sim.simx_opmode_oneshot_wait
            )
            yield [pyRTOS.timeout(15)]
            ...
            posicao = [9.0, 2.5, 1.0]
            sim.simxSetObjectPosition(
                clientID, mosquito[4], -1, posicao, sim.simx_opmode_oneshot_wait
            )
            yield [pyRTOS.timeout(2)]

            posicao = [7.0, 1.9, 1.0]
            sim.simxSetObjectPosition(
                clientID, mosquito[5], -1, posicao, sim.simx_opmode_oneshot_wait
            )
            yield [pyRTOS.timeout(5)]
    ...
    def task_fototransistores(self):
        ...
        for i in range(0, 13):
            [erro, aux1, aux2, aux3, aux4] = sim.simxReadProximitySensor(
                clientID, sensor[i], sim.simx_opmode_streaming
            )
        ...
        while True:
            ...
            for i in range(0, 13):
                [
                    ...
                    detectedSurfaceNormalVector[i],
                ] = sim.simxReadProximitySensor(
                    clientID, sensor[i], sim.simx_opmode_buffer
                )
            ...
            if detectou:
                self.send(
                    pyRTOS.Message(DETECTOU_PASSAGEM, self, "liga_ventoinha", detectou)
                )
            # End Work code
    ...
    def task_liga_ventoinha(self):
        ...
        while True:
            ...
            for msg in msgs:
                ...
                if msg.type == DETECTOU_PASSAGEM:
                    ...
                    sim.simxSetJointTargetVelocity(
                        clientID, ventoinha, 5.0, sim.simx_opmode_oneshot
                    )
            ...
            if (
                tempo_atual - tempo_ligamento
            ) > 10:  # 10 segundos se passaram desde o desligamento
                sim.simxSetJointTargetVelocity(
                    clientID, ventoinha, 0.0, sim.simx_opmode_oneshot
                )
    ...
    def task_liga_feromonio(self):
        ...
        while True:
            ...
            for msg in msgs:
                ...
                # Handle messages
                if msg.type == DESLIGA_FEROMONIO:
                    ligado = False
                    print("Desligando Feromonio!")
            ...
            if (
                tempo_atual - tempo_ligamento
            ) > 60:  # 30 segundos se passou desde o ligamento
                ligado = True
                print("Liberando Feromonio!")
                tempo_ligamento = time.time()
        ...
        while True:
            ...
            for msg in msgs:
                ...
                if msg.type == pyRTOS.QUIT:
                    # Tear down code here
                    self.send(pyRTOS.Message(DESLIGA_LED, self, "task_LED"))
                    sim.simxStopSimulation(clientID, sim.simx_opmode_oneshot_wait)
                    sim.simxAddStatusbarMessage(
                        clientID, "Sistema interrompido!", sim.simx_opmode_blocking
                    )
                    sim.simxSetJointTargetVelocity(
                        clientID, ventoinha, 0.0, sim.simx_opmode_oneshot
                    )
            ...
            # Work code here
            button = sim.simxGetInt32Signal(
                clientID, "myButton", sim.simx_opmode_buffer
            )
            if button == (0, 1):
                self.send(pyRTOS.Message(DESLIGA_LED, self, "task_LED"))
                sim.simxStopSimulation(clientID, sim.simx_opmode_oneshot_wait)
        ...
        while True:
            ...
            if eletrica_ligada == True:
                for i in range(0, 6):
                    ...
                    [error, checa_colisao] = sim.simxCheckDistance(
                        clientID,
                        pa_ventoinha,
                        corpo_mosquito[i],
                    ...
                    if checa_colisao < 0.5:
                        # matando o mosquito:
                        sim.simxRemoveModel(
                            clientID, corpo_mosquito[i], sim.simx_opmode_oneshot_wait
                        )
                        sim.simxRemoveObject(
                            clientID, mosquito[i], sim.simx_opmode_oneshot_wait
                        )
                        # print(str(i) + "colidiu!")
    ...
    pyRTOS.add_task(
        pyRTOS.Task(
            task_desliga_sistema,
            priority=3,
            name="desliga_sistema",
            notifications=None,
            mailbox=True,
        )
    )
